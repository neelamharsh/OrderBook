...
cout<<"Enter Name of Birthday Boy                 :"; gets(birthdayname);
cout<<"Father's Name                             :"; cin.getline(fname,20);
...
{
 ofstream fout;
 fout.open("cardnewa.txt",ios::binary|ios::out|ios::app);
 s.getData();
...
{
 ifstream fin;
 fin.open("cardnewa.txt",ios::in|ios::binary);
...
void deleteData()
{
 ...
 if(flag==0)
  cout<<"The Serial Number "<<n<<" not found....\n\n";
 remove("cardnewa.txt");
 rename("tempcard.txt","cardnewa.txt");
}
...
void modifyData()
{
 ...
 fio.open("cardnewa.txt",ios::in|ios::out|ios::binary);
